...
function togglePECourse(n,rank)
{
    var course_id = $("tr." + n + " td:eq(0)").attr("id");
    var current = $("." + n + ".btn");
    var current_grp = $("." + n + ".btn-group");
    var state = 0;
    if (current.hasClass("hide") || current.val() == "补选")
        state = 1;
    current.attr("disabled",true);
    if (sendPERequest(n,course_id,state,rank))
    {
        if ($("#schedule-result").text() != "")
        {
            $("#view-schedule-btn").trigger("click");
        ...
        if (state == 1)
        {
            ...
            current_grp.addClass("hide");
        }
        else
        {
            current.addClass("hide");
            current_grp.removeClass("hide");
        }
    }
    current.attr("disabled",false);
}
        ...
        success: function(msg,textStatus,jqXHR)
        {
            ...
            else
            {
                ...
                for (var i = 0;i < list.length;i++)
                {
                    ...
                    {
                        var rank_map = new Array("","(第一志愿)","(第二志愿)","(第三志愿)","(第四志愿)");
                        var rank = list[i].rank;
                        if (rank > 10000)
                            rank -= 10000;
                        $("#course-list").append("<tr class='" + index + "'><td id='"+ id + "'>" + course_name + rank_map[rank] + "<a class='withajaxpopover visible-phone' rel='popover' title='课程信息' data-content='类别：" + course_type + "<br>学分：" + credit + "<br>考核方式：" + exam + "<br>起止时间：" + period + "<br>当前人数：" + hastaken + "/" + capacity + "'><i class='icon-info-sign'></i></a></td><td class='hidden-phone'>" + course_type + "</td><td class='hidden-phone'>" + credit + "</td><td><a target='_blank' href='" + teacher.site + "' class='withajaxpopover' rel='popover' data-placement='bottom' title='教师信息' data-content='姓名：" + teacher.teacher_name + "<img src=" + teacher.img_addr + " width=68 height=68 style=float:right><br>学系：" + teacher.department.name + "<br>职称：" + teacher.title + "<br>主页：" + teacher.site + "'>" + teacher.teacher_name + "</a></td><td class='hidden-phone'>" + exam + "</td><td class='hidden-phone'>" + period + "</td><td class='" + index + " course-time-1'></td><td class='" + index + " course-locate-1 hidden-phone'></td><td class='hidden-phone'>" + hastaken + "/" + capacity + "</td><td><div class='select btn-group " + index + "'><button class='select btn btn-primary'>选课</button><button class='select btn btn-primary dropdown-toggle' data-toggle='dropdown'><span class='caret'></span></button><ul class='dropdown-menu'><li><a href='#' onclick='togglePECourse(" + index + "," + 1 + ")'>第一志愿</a></li><li><a href='#' onclick='togglePECourse(" + index + "," + 2 + ")'>第二志愿</a></li><li><a href='#' onclick='togglePECourse(" + index + "," + 3 + ")'>第三志愿</a></li><li><a href='#' onclick='togglePECourse(" + index + "," + 4 + ")'>第四志愿</a></li></ul></div><input type='button' class='select btn btn-primary hide " + index + "' onclick='togglePECourse(" + index + ")' value='选课'/></td></tr>");
                        for (var j = 0;j < course_time.length;j++)
                        {
                            week = week_map[course_time[j].week];
                            time = course_time[j].time;
                        ...
                        var current_button = $("." + index + ".btn");
                        var current_btn_grp = $("." + index + ".btn-group");
                        if (take == 0)
                        {
                            current_btn_grp.removeClass("hide");
                            current_button.addClass("hide");
                        }
                        else if (take == 1)
                        {
                            current_button.removeClass("hide").removeClass("btn-primary").removeClass("btn-inverse").removeClass("btn-success").addClass("btn-danger");
                            current_button.val("退课");
                            current_btn_grp.addClass("hide");
                        }
                        else if (take == 2)
                        {
                            current_button.removeClass("hide").removeClass("btn-primary").removeClass("btn-danger").removeClass("btn-inverse").addClass("btn-success");
                            current_button.val("补选");
                            current_btn_grp.addClass("hide");
                        }
                        else if (take == 3)
                        {
                            current_button.removeClass("hide").removeClass("btn-primary").removeClass("btn-danger").removeClass("btn-success").addClass("btn-inverse");
                            current_button.val("退筛选");
                            current_btn_grp.addClass("hide");
                        }
                    }
...
function sendPERequest(n,course_id,state,rank)
{
    var flag = false;
    $.ajax({
        url: '/student/toggleCourse/',
        data: 'course_id=' + course_id + '&state=' + state + '&rank=' + rank,
        datatype: 'json',
        type: 'post',
        async: false,
        error: function(jqXHR,textStatus,errorThrown)
        ...
        success: function(msg,textStatus,jqXHR)
        {
            ...
            {
                var rank_map = new Array("","(第一志愿)","(第二志愿)","(第三志愿)","(第四志愿)");
                var course = $("tr." + n + " td:eq(0)");
                var name = ((course.text()).split("("))[0];
                if (rank == null)
                    rank = 0;
                course.text(name + rank_map[rank]);
                var pos = ($("#course-result tr." + n + " td:eq(8)").text()).indexOf("/");
                var total = ($("#course-result tr." + n + " td:eq(8)").text()).substring(pos + 1);
                $("#course-result tr." + n + " td:eq(8)").text(hastaken + "/" + total);
                msg_area.empty();
